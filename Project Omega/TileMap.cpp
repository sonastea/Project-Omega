...
void TileMap::addTile(const int x, const int y, const int z, const sf::IntRect& texture_rect, const bool& collision, const short& type)
{
		...
		z < this->layers && z >= 0)
	{
		...
		/* DEBUGGING LOG */
		std::cout << "DEBUG: ADDED REGULAR TILE!" << "\n";
	}
}

void TileMap::addTile(const int x, const int y, const int z, const sf::IntRect& texture_rect, const int enemy_type, const int enemy_amount, const int enemy_tts, const int enemy_md)
{
	if (x < this->maxSizeWorldGrid.x && x >= 0 &&
		y < this->maxSizeWorldGrid.y && y >= 0 &&
		z < this->layers && z >= 0)
	...
	/* DEBUGGING LOG */
	std::cout << "DEBUG: ADDED ENEMYSPAWNER TILE!" << "\n";
}
...
void TileMap::updateWorldBoundsCollision(Entity* entity, const float& dt)
{
	...
	{
		entity->setPosition(sf::Vector2f(this->maxSizeWorldF.x - entity->getGlobalBounds().width, entity->getPosition().y));
		entity->stopVelocityX();
	}
	...
	{
		entity->setPosition(sf::Vector2f(entity->getPosition().x, this->maxSizeWorldF.y - entity->getGlobalBounds().height));
		entity->stopVelocityY();
	}
}
...
{
	/* TILES - ONLY TILES AROUND THE PLAYER */
	this->layer = 0;
...
void TileMap::updateTiles(Entity* entity, const float& dt, EnemySystem& enemySystem)
{
	...
	this->fromY = entity->getGridPosition(this->gridSizeI).x - 8;
	if (this->fromY < 0)
		this->fromY = 0;
	else if (this->fromY > this->maxSizeWorldGrid.y)
		this->fromY = this->maxSizeWorldGrid.y;

	this->toY = entity->getGridPosition(this->gridSizeI).x + 9;
	if (this->toY < 0)
		this->toY = 0;
	else if (this->toY > this->maxSizeWorldGrid.y)
		this->toY = this->maxSizeWorldGrid.y;
...
{
	return this->map[x][y][z][0]->getType() == type;
}
